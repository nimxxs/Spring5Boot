...
import nimxxs.hello.boot.spring5boot.controller.PdsController;
...
public class PdsUtils {
    ...
    public PdsAttach processUpload(MultipartFile attach) {
        ...
        // 파일명.split(".")[1]    이렇게만 쓰면 아래 파일명을 뽑아내지 못하게 됨
        // 파일명 : abc123.987xyz.zip -> 확장자 : zip
        // 파일명.substring(lastIndexOf"."+1)  맨 마지막 점을 기준으로 한 칸 더 가서 잘라야지 . 뒤로 지정이됨
        int pos = pa.getFname().lastIndexOf(".") + 1;
        pa.setFtype(pa.getFname().substring(pos));

        // 업로드할 파일 정보 알아내기 - 파일 크기
        pa.setFsize(attach.getSize()/1024 + "");        // 1GB = 1024MB
